...
public class MessageActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        reference = FirebaseDatabase.getInstance().getReference("Kullanicilar").child("Bireysel").child(userid);

        reference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                UsersBireysel user = dataSnapshot.getValue(UsersBireysel.class);
                username.setText(user.getAdi());
                if (user.getImageURL()==null){
                    profile_image.setImageResource(R.drawable.pikpng);
